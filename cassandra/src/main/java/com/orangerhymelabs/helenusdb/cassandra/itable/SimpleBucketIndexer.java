...
public class SimpleBucketIndexer
{
	 ...
	 */
	protected int computeBucketIndex(Long hash)
	{
		...
		if (index < 0)
		{
			//returned value of index == (-(insertion point) - 1)
			index = -(index + 2);
		}
...
//		index = index % size();
	...
	{
		long raw = Math.abs(type.asLong(key));
		...
		switch(type)
		{
			...
			break;
			case BOOLEAN:
				if (raw == 0L) raw = buckets.get(0);
				else raw = buckets.get(1);
			default:
			break;
		}
